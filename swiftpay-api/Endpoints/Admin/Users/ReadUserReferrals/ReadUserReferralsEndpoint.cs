...
public sealed class ReadUserReferralsEndpoint(
    PrimaryDbContext dbContext,
    IOptions<PlatformSettingsOptions> platformSettings,
    IStorageService storageService
) : Endpoint<ReadUserReferralsRequest, ReadUserReferralsResponse>
{
    public override void Configure()
    {
        Get("users/{userId:guid}/referrals");
        Group<AdminGroup>();
    }

    public override async Task HandleAsync(ReadUserReferralsRequest req, CancellationToken ct)
    {
        ...
        if (user == null)
        {
            await Send.ResponseAsync(new ReadUserReferralsResponse
            {
        ...
        var referredUsers = referredUsersRaw
            .Select(u =>
            {
                ...
                return new AdminReferredUserData
                {
                    Id = u.Id,
                    Name = u.Name,
                    Email = u.Email,
        ...
        var referralStartAt = user.ReferralCodeCreatedAt;
        DateTime? nextAllowedWithdrawalRequestAt = null;
        if (withdrawalIntervalValue > 0 && (referralStartAt.HasValue || lastWithdrawalRequestAt.HasValue))
        {
            var cooldownStartAt = lastWithdrawalRequestAt ?? referralStartAt!.Value;
        ...
        var withdrawalRequests = await dbContext.ReferralCommissionWithdrawalRequests
            ...
            .OrderByDescending(r => r.RequestedAt)
            .Take(20)
            .Select(r => new AdminReferralCommissionWithdrawalRequestData
            {
                Id = r.Id,
                Amount = r.Amount,
        ...
        var paymentHistory = new List<AdminReferralCommissionPaymentHistoryData>(paymentHistoryRaw.Count);
        foreach (var payment in paymentHistoryRaw)
        {
            ...
            paymentHistory.Add(new AdminReferralCommissionPaymentHistoryData
            {
                Id = payment.Id,
        ...
        await Send.OkAsync(new ReadUserReferralsResponse
        {
            Data = new AdminUserReferralsData
            {
                ...
                PayoutPixKey = user.ReferralPayoutPixKey,
