...
import { TextField, Label as HeroLabel, Select, ListBox, Chip, Modal } from '@heroui/react';
...
export function PixEstaticoContent({ merchantId }: { merchantId: string }) {
  ...
  const [selectedHistorical, setSelectedHistorical] = useState<MinimalPaymentLink | null>(null);
  const [historicalQr, setHistoricalQr] = useState<string | null>(null);
  const [historicalCopy, setHistoricalCopy] = useState<string | null>(null);
  ...
  const handleViewHistorical = async (link: MinimalPaymentLink) => {
    setSelectedHistorical(link);
    setHistoricalQr(null);
    setHistoricalCopy(null);
    setIsLoadingHistorical(true);
    try {
      const token = link.paymentLinkUrl.split('/').pop() ?? link.id;
      const startedRaw = await startPaymentLink(token, 'Pix');
      const startedData = (startedRaw as unknown as { data?: Record<string, unknown> | null })?.data ?? null;
      const pixObj = startedData && typeof startedData === 'object' && ('pix' in startedData || 'Pix' in startedData) ? ((startedData as Record<string, unknown>)['pix'] ?? (startedData as Record<string, unknown>)['Pix']) as Record<string, unknown> | null : null;
      const qrVal = startedData && typeof startedData === 'object' && ('qr' in startedData || 'Qr' in startedData) ? ((startedData as Record<string, unknown>)['qr'] ?? (startedData as Record<string, unknown>)['Qr']) as string | null : null;
      const copyVal = startedData && typeof startedData === 'object' && ('copyAndPaste' in startedData || 'CopyAndPaste' in startedData) ? ((startedData as Record<string, unknown>)['copyAndPaste'] ?? (startedData as Record<string, unknown>)['CopyAndPaste']) as string | null : null;
      const pixQr = pixObj && typeof pixObj === 'object' && ('qrCode' in pixObj || 'QrCode' in pixObj) ? ((pixObj as Record<string, unknown>)['qrCode'] ?? (pixObj as Record<string, unknown>)['QrCode']) as string | null : null;
      const pixCopy = pixObj && typeof pixObj === 'object' && ('copyAndPaste' in pixObj || 'CopyAndPaste' in pixObj) ? ((pixObj as Record<string, unknown>)['copyAndPaste'] ?? (pixObj as Record<string, unknown>)['CopyAndPaste']) as string | null : null;
      setHistoricalQr(qrVal ?? pixQr ?? null);
      setHistoricalCopy(copyVal ?? pixCopy ?? null);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Erro ao carregar QR');
    } finally {
  ...
  return (
    <div className="grid gap-6">
          ...
          {isLoadingList ? (
            ...
            <div className="grid gap-2">
              {staticLinks.map((link) => {
                ...
                return (
                  <div key={link.id} onClick={() => handleViewHistorical(link)} className="flex cursor-pointer items-center justify-between gap-2 rounded-3 border border-white/12 bg-black/40 px-3 py-2 hover:bg-white/5">
                    <div className="flex items-center gap-2">
                      <Chip variant="soft" color={mapParseColorToChipColor(parse.color)} size="sm" className="gap-1">
                        {parse.icon}
                        {parse.label}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="icon-xs" onClick={(e) => { e.stopPropagation(); handleDelete(link.id); }} disabled={deletingId === link.id} aria-label="Excluir QR">
      ...
      <Modal.Backdrop isOpen={!!selectedHistorical} onOpenChange={(open) => { if (!open) { setSelectedHistorical(null); setHistoricalQr(null); setHistoricalCopy(null); } }} isDismissable>
        <Modal.Container size="lg" placement="center">
          <Modal.Dialog>
            <Modal.CloseTrigger />
            <Modal.Header>
              <Modal.Heading>QR Estático</Modal.Heading>
              <p className="text-sm text-muted">{selectedHistorical ? formatCurrency(selectedHistorical.amount) : ''} — {selectedHistorical?.id}</p>
            </Modal.Header>
            <Modal.Body>
              {isLoadingHistorical ? (
                <p className="text-sm text-muted">Carregando QR...</p>
              ) : historicalQr || historicalCopy ? (
                <div className="grid gap-4">
                  <div className="w-fit rounded-xl border border-white/12 bg-white p-3">
                    {historicalQr && (historicalQr.startsWith('data:') || historicalQr.startsWith('http')) ? (
                      <img alt="QR Code" src={historicalQr} className="h-auto w-full max-w-64" />
                    ) : (
                      <QRCodeSVG value={historicalCopy ?? historicalQr ?? 'pix'} size={240} level="L" />
                    )}
                  </div>
                  {historicalCopy && (
                    <div className="grid gap-2">
                      <Label>Copia e cola</Label>
                      <Input readOnly value={historicalCopy} className="border-white/12 bg-black/40 text-white" />
                      <Button variant="secondary" onClick={async () => { await navigator.clipboard.writeText(historicalCopy); toast.success('Copiado'); }} className="w-fit gap-2">
                        <RiCheckLine className="size-3.5" /> Copiar
                      </Button>
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-sm text-muted">Não foi possível carregar o QR.</p>
              )}
            </Modal.Body>
          </Modal.Dialog>
        </Modal.Container>
