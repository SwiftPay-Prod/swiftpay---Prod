	...
	return (
		<section id="pricing" className="relative py-16 sm:py-24 bg-[#000000] border-t border-white/10 text-white">
			<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
				...
				{/* Calculator Box */}
				<div className="max-w-4xl mx-auto rounded-2xl border border-white/12 bg-card p-6 sm:p-10 space-y-8">
					...
					<div className="space-y-4">
						...
						<input
							...
							className="w-full h-2.5 bg-surface-deep rounded-lg appearance-none cursor-pointer accent-white"
						/>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-white/10">
						...
						<div className="rounded-xl border border-white/8 bg-surface-deep p-5 space-y-3">
							<div className="text-xs font-bold text-white/40 uppercase tracking-wider">
								Outros Gateways & Adquirentes
							</div>
							<div className="text-xl font-extrabold font-mono text-danger tabular-nums">
						...
						{/* SwiftPay Savings */}
						<div className="rounded-xl border border-success/30 bg-success/5 p-5 space-y-3 shadow-[0_0_20px_rgba(0,168,126,0.06)]">
							<div className="text-xs font-bold text-success uppercase tracking-wider font-mono">
								Com a SwiftPay (Economia Estimada)
							</div>
							<div className="text-2xl font-extrabold font-mono text-white tabular-nums">
								Economia de <span className="text-success">{formatCurrency(monthlySavings)}</span> / mês
							</div>
							<div className="text-xs font-semibold text-success font-mono">
								~ {formatCurrency(annualSavings)} economizados por ano!
							</div>
							<ul className="text-xs text-white/80 font-medium space-y-1.5 pt-1">
								<li className="flex items-center gap-1.5">
									<Icon icon={CheckmarkCircle02Icon} className="w-3.5 h-3.5 text-success" />
					...
					<div className="pt-2 flex justify-center">
						<button
							...
							className="group inline-flex items-center justify-center gap-2 rounded-full bg-white px-8 py-3.5 text-sm font-bold text-black hover:bg-white/90 transition-all cursor-pointer"
						>
