															...
															<div className="flex flex-1 flex-col gap-1 min-w-0">
																...
																<div className="flex flex-wrap items-center gap-1.5">
																	{acquirer.operationTypes?.map((type) => {
																		...
																		return parsed ? (
																			<Chip
																				key={type}
																				size="sm"
																				className={`gap-0.5 text-xs h-5 ${parsed.className ?? ''}`}
																			>
																				{parsed.icon}
																				{parsed.label}
																			</Chip>
																	...
																	{acquirer.supportsPix && (
																		<Chip size="sm" className="h-5 gap-0.5 text-xs bg-success/10 text-success border-success-soft-hover">
										...
										<div className="flex min-w-0 flex-col gap-1">
											...
											<div className="flex flex-wrap items-center gap-1.5">
												{selectedAcquirer.operationTypes?.map((opType) => {
													...
													return parsed ? (
														<Chip key={opType} size="sm" className={`h-5 gap-0.5 text-xs ${parsed.className}`}>
															{parsed.icon}
															{parsed.label}
														</Chip>
													) : null;
