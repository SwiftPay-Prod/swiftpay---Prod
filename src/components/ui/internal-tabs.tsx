...
export function InternalTabs({
	...
	const baseListClassName = isVertical
		? 'flex-col w-full *:h-8.5 *:w-full *:justify-start *:px-3 *:text-xs *:font-medium *:whitespace-nowrap rounded-lg bg-surface/50 p-1 border border-border/80'
		: 'w-full flex items-center gap-1 *:h-8.5 *:w-fit *:px-3 *:text-xs *:whitespace-nowrap rounded-lg bg-surface/50 p-1 border border-border/80 overflow-x-auto scrollbar-none';
		...
		<Tabs.List aria-label={ariaLabel} className={baseListClassName}>
			{items.map((item) => (
				<Tabs.Tab
					key={item.id}
					id={item.id}
					className={`group relative flex items-center justify-center rounded-md px-3 py-1.5 text-xs font-medium transition-colors cursor-pointer border border-transparent focus:outline-none text-muted-foreground hover:text-foreground hover:bg-surface/60 data-[selected=true]:bg-card data-[selected=true]:text-foreground data-[selected=true]:border-border/80 data-[selected=true]:font-semibold data-[selected=true]:shadow-2xs ${!isVertical && !visibleIdSet.has(item.id) ? 'hidden' : ''}`}
				>
