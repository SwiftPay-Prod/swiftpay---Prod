...
function MethodStatusBadge({ enabled, label }: { enabled: boolean; label: string }) {
  return (
    <div className="flex items-center justify-between rounded-xl border border-divider bg-content1 px-3 py-3 transition-colors duration-200 hover:border-default/30 hover:bg-content2/40">
...
function ChannelFeeCard({
  ...
  return (
    <div className="rounded-xl border border-divider bg-content1 p-4 transition-colors duration-200 hover:border-default/30 hover:bg-content2/40">
...
function MethodGlobalLimitsCard({
  ...
  return (
    <div className="rounded-xl border border-divider bg-content1 p-4 transition-colors duration-200 hover:border-default/30 hover:bg-content2/40">
      <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">Limites Globais de {methodLabel}</p>
      <div className="flex flex-col">
...
function CompensationCard({
  ...
  return (
    <div className="rounded-xl border border-divider bg-content1 p-4 transition-colors duration-200 hover:border-default/30 hover:bg-content2/40">
      <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">Compensação da Reserva ({methodLabel})</p>
      <div className="flex flex-col">
        <div className="flex items-center gap-3 rounded-lg border border-divider bg-surface-secondary p-3">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-content2">
            <Icon icon={StopWatchIcon} className={`icon-xs ${compensationToneClass}`} />
          </div>
          <div className="flex min-w-0 flex-col gap-0.5">
            <span className="text-xs text-muted">Prazo de liberação</span>
            <span className={`text-sm font-semibold ${compensationToneClass}`}>{`D+${days}`}</span>
          </div>
        </div>
        {shouldShowReserve && (
          <div className="mt-3 flex items-center gap-3 rounded-lg border border-divider bg-surface-secondary p-3">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-content2">
              <Icon icon={PercentSquareIcon} className={`icon-xs ${compensationToneClass}`} />
            </div>
            <div className="flex min-w-0 flex-col gap-0.5">
              <span className="text-xs text-muted">Reserva financeira</span>
