...
import { clearPWACache, unregisterServiceWorkers, clearLocalStorage } from '@/utils/pwa-utils';
...
export function DevToolsContent() {
	...
	async function handleReloadPWA() {
		if (!confirm('Isso vai limpar o cache do PWA e recarregar a página. Continuar?')) return;

		addLog('=== PWA RELOAD ===', 'info');
		addLog('Limpando caches...', 'info');

		const cachesCleared = await clearPWACache();
		addLog(`Caches removidos: ${cachesCleared}`, cachesCleared > 0 ? 'success' : 'info');

		const swUnregistered = await unregisterServiceWorkers();
		addLog(`Service Workers removidos: ${swUnregistered}`, swUnregistered > 0 ? 'success' : 'info');

		clearLocalStorage();
		addLog('LocalStorage limpo (FCM tokens)', 'success');

		addLog('Recarregando página em 1s...', 'info');
		toast('Cache limpo', {
			description: 'O cache foi limpo. Recarregando a página...',
			indicator: <Icon icon={CheckmarkCircle02Icon} className="icon-sm" />,
			variant: 'success',
		});
