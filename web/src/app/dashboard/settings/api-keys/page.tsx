...
export default function ApiKeysPage() {
  ...
  const createKey = useMutation({
    mutationFn: (name: string) =>
      apiKeys.create({ name, merchantId: '00000000-0000-0000-0000-000000000000' }),
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-6 space-y-4">
        ...
        <div className="flex gap-3">
          <input type="text" value={name} onChange={e => setName(e.target.value)}
            placeholder="Nome da chave (ex: Produção, Homologação)"
            className="flex-1 px-3 py-2 border border-zinc-300 rounded-lg outline-none focus:border-black" />
          <button onClick={() => name && createKey.mutate(name)} disabled={!name || createKey.isPending}
                ...
                <div key={key.id} className="p-4 flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <button onClick={() => setShowKey(showKey === key.id ? null : key.id)} className="text-zinc-400 hover:text-black">
                        {showKey === key.id ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                      </button>
                      <button onClick={() => copyToClipboard(key.key)} className="text-zinc-400 hover:text-black">
                        <Copy className="h-4 w-4" />
                      </button>
                    </div>
                    <p className="text-xs text-zinc-400 mt-1">Escopos: {key.scopes || 'read,write'}</p>
                  </div>
                  <button onClick={() => deleteKey.mutate(key.id)} className="text-red-400 hover:text-red-600">
